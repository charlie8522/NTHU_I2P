	...
	/* 無窮迴圈  裡面包含了這個程式的主要工作
   	...
   	while (!done) {
        ...
        for (i=0; i<L; i++) {
      	...
      	for (i = 0; i < HEIGHT; i++) {
         	for (j = 0; j < WIDTH; j++) {
                drawChar(a[i][j],j + OFFSET_X, i + OFFSET_Y,7,14);
         	}
      	}
      	for (i=0; i<L; i++) {
            drawChar(pl_icon[pl[i].direction],pl[i].col + OFFSET_X, pl[i].row + OFFSET_Y,8,i*2+10);
      	}
      	/* current location */


        sprintf(str, "Round: %05d", round++);
        drawString(str,2,16,0,11);
        for (i=0; i<L; i++) {
            sprintf(str, "Vital: %03d, Chi: %03d", pl[i].vital, pl[i].chi);
            drawString(str,2,17+i,0,i*2+10);
        }
